...
void CalculateQpMats(const MatrixXd& a_exp, const MatrixXd& b_exp,
    ...
    // We construct the P matrix by filling in h x h sub-matrices, each with size
    // action_dim x action_dim.
    // The r_th (r in [1, h]) diagonal sub-matrix of P is:
    // 2 * sum_{i=0:h-r}(B'A'^i L A^i B) + alpha, where h is the horizon.
    // The off-diagonal sub-matrix at row r and column c of P is:
...
MatrixXd AsBlockDiagonalMat(const std::vector<double>& qp_weights,
    ...
    // asDiagonal not really copying the memory. Creates the temporary will ensure
    // copy on return.
