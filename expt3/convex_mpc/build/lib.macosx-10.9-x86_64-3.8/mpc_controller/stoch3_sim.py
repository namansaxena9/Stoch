...
URDF_NAME = "./stoch3_description/urdf/stoch3.urdf"
START_POS = [0, 0, 0.45]

MPC_BODY_MASS = 25
MPC_BODY_INERTIA = np.array((0.2472, 0, 0, 0, 0.5664, 0, 0, 0, 0.7926))

# MPC_BODY_INERTIA = np.array((0.05922, 0, 0, 0, 0.06835, 0, 0, 0, 0.56123))
...
HIP_NAME_PATTERN = re.compile(r"\w+_abd_\w+")
UPPER_NAME_PATTERN = re.compile(r"\w+_hip_\w+")
LOWER_NAME_PATTERN = re.compile(r"\w+_knee_\w+")
TOE_NAME_PATTERN = re.compile(r"\w+_ankle_\w+")
...
# LAIKAGO_DEFAULT_HIP_ANGLE = 0.0 #0.772001
# LAIKAGO_DEFAULT_KNEE_ANGLE = 0.0 #-1.4129

NUM_LEGS = 4
NUM_MOTORS = 12

# Bases on the readings from Laikago's default pose.
INIT_MOTOR_ANGLES = np.array([-0.007,  1.035, -1.794,  0.006,  1.033, -1.794, -0.,     0.985, -1.801,  0.002,  0.983, -1.803])
MOTOR_NAMES = [
    "fl_abd_joint",
    "fl_hip_joint",
    "fl_knee_joint",
...
class SimpleRobot(object):
  def __init__(self, pybullet_client, robot_uid, simulation_time_step, add_init_weight=False):
    ...
    if add_init_weight:
      self.weight = self._add_weight_block(mass=2)
  ...
  def ResetPose(self):
    ...
    for name, i in zip(MOTOR_NAMES, range(len(MOTOR_NAMES))):
      if "abd_joint" in name:
        angle = INIT_MOTOR_ANGLES[i] + HIP_JOINT_OFFSET
      elif "hip_joint" in name:
  ...
  def _add_weight_block(self, mass):
    # Define the properties of the weight
    weight_mass = mass  # mass of the weight in kg
    weight_size = [0.1, 0.1, 0.1]  # dimensions of the weight in meters

    # Create the weight as a box shape
    weight_id = self.pybullet_client.createCollisionShape(self.pybullet_client.GEOM_BOX, halfExtents=weight_size)
    # weight_id = self.pybullet_client.createCollisionShape(self.pybullet_client.GEOM_SPHERE, radius=0.2)
    # weight_id = self.pybullet_client.createCollisionShape(self.pybullet_client.GEOM_CYLINDER, radius=0.1, height=0.1)

    # Calculate the center of mass of the weight
    weight_com = [0, 0, -weight_size[2] / 2]

    # Add the weight to the simulation
    weight_body_id = self.pybullet_client.createMultiBody(weight_mass, weight_id, weight_id, [0, 0, 1],
                                                          [0, 0, 0, 1], weight_com)

    # Set the color of the weight block
    color = [0, 1, 1, 1]  # Red color
    self.pybullet_client.changeVisualShape(weight_body_id, -1, rgbaColor=color)

    # Position the weight on top of the quadruped torso
    quadruped_pos, _ = self.pybullet_client.getBasePositionAndOrientation(self.quadruped)

    # Create a fixed joint between the weight block and the torso
    self.pybullet_client.createConstraint(self.quadruped, -1, weight_body_id, -1,
                                          self.pybullet_client.JOINT_FIXED, quadruped_pos, [0, 0, 0], [0, 0, 0])

    return weight_body_id
  ...
  def GetBasePosition(self):
    """Get the linear position of minitaur's base.

    Returns:
      The position of minitaur's base.
    """
    position, _ = self.pybullet_client.getBasePositionAndOrientation(self.quadruped)
    return position
  ...
  def Step(self, action):
    ...
    for i in range(ACTION_REPEAT):
      ...
      self._cam_dist = 1.
      pos, _ = (self.pybullet_client.getBasePositionAndOrientation(self.quadruped))
      self.pybullet_client.resetDebugVisualizerCamera(self._cam_dist, 30, 0, pos)
